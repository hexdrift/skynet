...
export function AppShell({ children }: { children: React.ReactNode }) {
 ...
 dir="ltr"
 style={{
 borderImage: "linear-gradient(to right, transparent, var(--border) 20%, var(--border) 80%, transparent) 1",
 }}
 >
 ...
 {session?.user && (
 <>
 <span className="hidden sm:inline text-xs text-muted-foreground">{session.user.name ?? session.user.email}</span>
 <button
 type="button"
