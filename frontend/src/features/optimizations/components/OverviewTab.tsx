...
const ScoreChart = dynamic(
  () => import("@/components/score-chart").then((m) => m.ScoreChart),
  ...
  return (
    <>
      ...
      {job.optimization_type !== "grid_search" && (
        <FadeIn delay={0.05}>
          {(() => {
            const completedStageIdx = job.status === "success"
            ...
            return (
              <div className="relative flex items-start justify-between" dir="rtl" data-tutorial="pipeline-stages">
                ...
                <div
                  ...
                  style={{ width: `calc(${(Math.min(completedStageIdx, PIPELINE_STAGES.length - 1) / (PIPELINE_STAGES.length - 1)) * 100}% - 28px)` }}
                />
                {PIPELINE_STAGES.map((s, i) => {
                  ...
                  return (
                    <div key={s.key} className="relative z-10 flex flex-col items-center gap-2 min-w-0 group/node cursor-pointer" onClick={() => onStageClick(s.key)}>
          ...
          <StaggerContainer className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <StaggerItem>
              <TiltCard className=" rounded-xl border border-border/50 bg-card p-6 text-center">
                ...
                <p className="text-3xl font-mono font-bold">{formatPercent(job.result.baseline_test_metric)}</p>
              </TiltCard>
            </StaggerItem>
            <StaggerItem>
              <TiltCard className="rounded-xl border border-primary/30 bg-gradient-to-br from-primary/5 to-primary/10 p-6 text-center shadow-[0_0_20px_rgba(var(--primary),0.08)]">
                ...
                <p className="text-3xl font-mono font-bold text-primary">{formatPercent(job.result.optimized_test_metric)}</p>
              </TiltCard>
            </StaggerItem>
            <StaggerItem>
              <TiltCard className={`rounded-xl border p-6 text-center ${(job.result.metric_improvement ?? 0) >= 0 ? "border-stone-400/50 bg-gradient-to-br from-stone-100/50 to-stone-200/30" : "border-red-300/50 bg-gradient-to-br from-red-50/50 to-red-100/30"}`}>
                ...
                <p className={`text-3xl font-mono font-bold ${(job.result.metric_improvement ?? 0) >= 0 ? "text-stone-600" : "text-red-600"}`}>{formatImprovement(job.result.metric_improvement)}</p>
              </TiltCard>
            </StaggerItem>
          </StaggerContainer>
        </div>
      ...
      {job.status === "success" && job.optimization_type === "grid_search" && job.grid_result && activePairIndex === null && (
        <GridOverview job={job} onPairSelect={onPairSelect} />
      )}
    </>
  );
}
