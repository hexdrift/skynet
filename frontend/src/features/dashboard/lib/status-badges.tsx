...
export function formatScore(job: OptimizationSummaryResponse): React.ReactNode {
  ...
  if (optimized != null) return <span className="font-medium text-xs">{formatPercent(optimized)}</span>;
  if (baseline != null) return <span className="text-muted-foreground text-xs">{formatPercent(baseline)}</span>;
  return "-";
}
