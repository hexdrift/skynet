...
export type ChartData = {
  ...
  timelineData: { name: string; אופטימיזציות: number }[];
...
export function transformChartData(
  ...
  const improvement = analyticsData.top_improvement.map((j) => {
    ...
    return {
      ...
      delta:
        j.metric_improvement != null ? toPct(j.metric_improvement) : undefined,
    };
  });
  ...
  const topJobs =
    analyticsData.top_jobs_by_improvement as unknown as OptimizationSummaryResponse[];
  ...
  const datasetVsImprovement = analyticsData.dataset_vs_improvement.map(
    (j) => ({
  ...
  const efficiencyJobIds = analyticsData.efficiency.map(
    (j) => j.optimization_id,
  );
