...
async function getAllRects(page: Page): Promise<{ btn: Rect | null; drop: Rect | null }> {
  ...
  return page.evaluate(() => {
    const btn = document.querySelector(
      'button[aria-label^="סינון עמודת"][class*="text-primary"], button[aria-label^="סינון עמודת"]:focus, button[aria-label^="סינון עמודת"]',
    ) as HTMLElement | null;
    ...
    const drop = document.querySelector(
      "body > div.fixed.z-50.rounded-\\[22px\\]",
    ) as HTMLElement | null;
...
test.describe("Excel filter dropdown pinning", () => {
  ...
  for (const viewport of BREAKPOINTS) {
    test(`logs table: stays pinned during window + inner-container scroll @${viewport.width}x${viewport.height}`, async ({
      browser,
    }) => {
      await withAuthenticatedPage(browser, storageState, viewport, async (page) => {
        ...
        if (windowScrolled > 0 && afterWindow.stillOpen) {
          expect(
            Math.abs(afterWindow.delta - initial.delta),
            `window scroll delta @${viewport.width}`,
        ...
        if (innerScrolled > 0 && afterInner.stillOpen) {
          expect(
            Math.abs(afterInner.delta - baseline2.delta),
            `inner scroll delta @${viewport.width}`,
          ).toBeLessThanOrEqual(TOLERANCE);
        }
      });
    });
    ...
    await withAuthenticatedPage(
      ...
      async (page) => {
        ...
        await page.evaluate(() => {
          ...
          document
            .querySelectorAll<HTMLElement>(".overflow-auto,.overflow-y-auto")
            .forEach((el) => {
  ...
  test(`existing functionality: opens, search, checkbox, apply, clear, close on outside click`, async ({
    browser,
  }) => {
    await withAuthenticatedPage(
      browser,
  ...
  test(`dashboard optimizations table: stays pinned during window scroll`, async ({ browser }) => {
    await withAuthenticatedPage(
      ...
      async (page) => {
        ...
        if (after.stillOpen) {
          expect(
            Math.abs(after.delta - initial.delta),
            "dashboard window scroll delta",
          ).toBeLessThanOrEqual(TOLERANCE);
        }
      },
    );
