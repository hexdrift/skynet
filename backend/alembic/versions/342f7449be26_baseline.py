...
def _create_job_embeddings() -> None:
    """Create the pgvector extension, job_embeddings table and its indexes.

    Invoked only for the semantic search backend. Lexical and bm25 deployments
    run on a vanilla Postgres and skip this, so ``CREATE EXTENSION vector`` and
    the Vector/HNSW schema never execute on a database without pgvector.
    """
...
def _drop_job_embeddings() -> None:
    """Drop the job_embeddings indexes and table created by the semantic profile.

    Guarded by the same backend check as creation so a lexical/bm25 downgrade
    doesn't try to drop a table that was never created.
    """
...
def upgrade() -> None:
    ...
    # pgvector schema only for the semantic backend; lexical/bm25 run on a
    # vanilla Postgres and never issue CREATE EXTENSION vector.
    if embeddings_schema_enabled():
        _create_job_embeddings()
